...
export function ChatDrawer() {
  ...
  // Sync active project
  useEffect(() => {
    if (activeProjectId) setProjectId(String(activeProjectId));
  }, [activeProjectId]);
  ...
  const {
    ...
    queryKey: ["chat-conversations", currentProjectId],
    queryFn: async () =>
      requireApiResult(
        await chatApi.getProjectConversations(Number(currentProjectId)),
        "Could not load conversations",
      ) ?? [],
    enabled: isDrawerOpen && !!session?.token && !!currentProjectId,
    staleTime: 10_000,
    refetchInterval: isDrawerOpen ? 15_000 : false,
  });
  ...
  const {
    ...
    queryKey: ["chat-messages", selectedConv?.conversationId],
    queryFn: async () =>
      requireApiResult(
        await chatApi.getMessages(selectedConv!.conversationId),
        "Could not load messages",
      ) ?? [],
    enabled: !!selectedConv && view === "thread",
    staleTime: 5_000,
  ...
  // Mark conversation as read when viewing thread
  useEffect(() => {
    if (selectedConv && view === "thread") {
      chatApi.markRead(selectedConv.conversationId).catch(() => {});
    }
  }, [selectedConv, view]);

  const openThread = (conv: ConversationResponse) => {
  ...
  const sendMessage = async () => {
    ...
    setSending(true);
    try {
      const res = await chatApi.sendMessage(selectedConv.conversationId, {
        body: compose.trim(),
      });
      if (res.isSuccess) {
        setCompose("");
        ...
        toast.error(res.errorMessage ?? "Could not send message");
      }
    } catch {
  ...
  const createConversation = async () => {
    ...
    try {
      ...
      if (res.isSuccess) {
        ...
        toast.error(res.errorMessage ?? "Could not create conversation");
      }
    } catch {
      toast.error("Could not reach the backend");
    } finally {
  ...
  return (
    <Sheet open={isDrawerOpen} onOpenChange={(open) => !open && closeChatDrawer()}>
      <SheetContent
        side="right"
        className="flex w-full sm:w-[420px] flex-col gap-0 border-l bg-card p-0"
      >
        <SheetTitle className="sr-only">Team Chat</SheetTitle>
        ...
        <div className="flex h-14 items-center gap-2 border-b px-4 shrink-0">
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={closeChatDrawer}>
            <X className="h-4 w-4" />
          </Button>
        </div>
